OwlDB는 클라우드와 온프레미스 환경에서 데이터베이스를 설치·등록하고 통합 관리하는 관리형 데이터베이스 서비스입니다. 이 페이지에서는 OwlDB의 운영 환경과 주요 기능, 지원 엔진·토폴로지를 확인합니다.
...
고객이 자체 보유한 서버, 네트워크, 스토리지 등 물리적 인프라 자원을 기반으로 데이터베이스를 운영하는 방식입니다. 고정된 인프라 리소스를 활용하도록 설계되었으며, 외부 네트워크와 단절된 폐쇄망 환경을 지원합니다. OwlDB를 통해 호스트에 신규 데이터베이스를 설치하거나, 기존에 이미 운영 중인 데이터베이스를 OwlDB 관리 대상으로 연동하여 통합 제어할 수 있습니다.

{% hint style="info" %}
**참고**

자세한 내용은 온프레미스 가이드를 확인합니다.
{% endhint %}

# 주요 기능

OwlDB는 두 환경에서 공통으로 사용되는 관리 기능을 바탕으로, 클라우드와 온프레미스 각각의 인프라 특성에 맞는 전용 기능을 제공합니다.

## 공통 기능

| eClRr3NUk2Ht | ekmZY08XIUkm |
| --- | --- |
| **기능** | **설명** |
...
| TlmYuJolzVaa | UxBO6gchAdVX |
| --- | --- |
| **기능** | **설명** |
| **자동화된 프로비저닝** | 클라우드 자원 생성부터 데이터베이스 아키텍처 구성까지 전 과정 자동화 |
| **유연한 리소스 변경** | 워크로드 증감에 맞춘 인스턴스 사양·스토리지 확장 및 변경 |
| **클라우드 스냅샷 백업** | CSP 스냅샷 기능 연동 기반 백업·복구 |

## 온프레미스 특화

| 36L8GzCXz8cy | RMdKTSUrYJ7Z |
| --- | --- |
| **기능** | **설명** |
