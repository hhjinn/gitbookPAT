...
Barman Agent의 유닛 이름과 설정 파일 경로는 OwlDB에 고정되어 있어 변경할 수 없습니다. 다른 이름이나 경로를 사용하면 연동이 실패하므로 [4. Barman Agent 등록](#4-barman-agent-%EB%93%B1%EB%A1%9D)의 값을 그대로 사용해주십시오.
...
`barman_home` 은 설정한 값을 OwlDB가 연동 시점에 조회하여 사용합니다. 백업 데이터는 `<barman_home>/<데이터베이스 ID>` 에 저장됩니다. 단 [4번](#4-barman-agent-%EB%93%B1%EB%A1%9D)의 Agent 설정 파일 경로는 `barman_home` 과 무관하게 고정입니다.
...
cron은 최소한의 PATH 환경에서 실행되므로, `barman cron` 이 `pg_receivewal` 을 찾으려면 [3번](#3-barman-%EC%A0%84%EC%97%AD-%EC%84%A4%EC%A0%95)의 `path_prefix` 가 설정되어 있어야 합니다.
...
Barman 서버에 개인키를 배치합니다. 경로는 [6-2](#6-2-owlagentenv-%EC%84%A4%EC%A0%95)의 `BARMAN_SSH_KEYPATH` 와 같아야 합니다.
...
| SSH 접속 설정 | 데이터베이스 서버 `~/.ssh/config` | Barman 서버 접속 시 사용할 키 지정 (키 등록은 [7번](#7-ssh-%ED%82%A4-%EB%93%B1%EB%A1%9D) 참고) |
...
Barman 서버와 데이터베이스 서버 간 SSH 키가 등록되지 않았습니다. OwlDB는 접속 가능 여부를 검증하지 않으므로 연동 자체는 성공합니다. [7번](#7-ssh-%ED%82%A4-%EB%93%B1%EB%A1%9D)을 참고하여 양방향으로 키를 등록하고 아래 명령으로 접속을 확인합니다.
...
WAL 수집 방식을 ARCHIVER 또는 BOTH로 선택했으나 데이터베이스 서버에 `barman-cli` 가 설치되지 않았습니다. [8번](#8-%EB%8D%B0%EC%9D%B4%ED%84%B0%EB%B2%A0%EC%9D%B4%EC%8A%A4-%EC%84%9C%EB%B2%84-%EC%84%A4%EC%A0%95)을 참고하여 `3.11.1` 버전으로 설치합니다.
