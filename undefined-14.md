...
{% hint style="warning" %}
**주의** Standby(Recovery) 인스턴스가 선택된 상태에서는 파라미터를 수정할 수 없습니다. 수정하려면 Primary 인스턴스를 선택해야 합니다.
{% endhint %}
...
{% hint style="info" %}
**참고** 인스턴스 상태가 `Unavailable`인 경우, 현재값은 표시되지 않으며 Config 값이 대신 표시됩니다.
{% endhint %}

{% hint style="warning" %}
**주의** Standby(Recovery) 인스턴스를 선택한 경우 파라미터 수정이 제한됩니다. 파라미터를 수정하려면 Primary 인스턴스를 선택하세요.
{% endhint %}

{% hint style="info" %}
**참고** OpenSQL **OpenHA** 탭에서 `pg_hba`와 `slot` 파라미터는 조회하거나 수정할 수 없습니다. 해당 파라미터는 **연결 정보 관리** 메뉴에서만 설정합니다.
{% endhint %}
...
{% hint style="warning" %}
**주의** 수정 모드에서 저장하지 않고 화면을 벗어나면 변경 사항이 저장되지 않습니다. 수정 중임을 알리는 배너가 화면 상단에 표시됩니다.
{% endhint %}
...
3. **수정** 버튼을 클릭합니다.
4. 파라미터 값을 수정합니다. 특정 파라미터를 원래 값으로 되돌리려면 해당 파라미터를 선택한 후 **기본값 복원** 또는 **현재값 복원** 버튼을 클릭합니다. **참고** 파란색으로 강조된 파라미터 이름은 현재 수정 중인 값입니다.
...
{% hint style="info" %}
**참고** Tibero 다중 노드 구성에서 글로벌 파라미터는 수정할 수 없습니다.
{% endhint %}
...
{% hint style="info" %}
**참고** 템플릿 불러오기는 Tibero 엔진에서만 지원합니다. OpenSQL에서는 **불러오기** 버튼이 표시되지 않습니다.
{% endhint %}
...
{% hint style="info" %}
**참고** 파라미터 템플릿은 Tibero 엔진에서만 사용할 수 있습니다. OpenSQL 엔진을 사용하는 경우 파라미터 템플릿 메뉴가 표시되지 않습니다.
{% endhint %}
...
{% hint style="info" %}
**참고** 기본 제공 템플릿(OLAP, OLTP)은 수정하거나 삭제할 수 없습니다.
{% endhint %}
...
{% hint style="warning" %}
**주의** 수정 모드 진입 후 **저장** 버튼을 클릭하지 않으면 변경사항이 저장되지 않습니다.
{% endhint %}
