...
{% hint style="info" %} **참고**
OpenSQL 데이터베이스 관리는 Azure 환경에서만 사용할 수 있습니다. AWS 환경에서는 지원되지 않습니다. {% endhint %}

{% hint style="info" %} **참고**
DB 엔진이 Tibero로 설정된 경우에는 이 페이지 대신 테이블스페이스 및 데이터 파일 관리 화면이 표시됩니다. {% endhint %}
...
{% hint style="info" %} **참고**
Tuple Health는 추정 통계 기반 가이드로, 트래픽 패턴에 따라 실제 성능 영향이 다를 수 있습니다. {% endhint %}

검색창에 별칭을 입력해 특정 데이터베이스를 찾을 수 있습니다. 열 헤더를 클릭하면 오름차순/내림차순으로 정렬하며, 기본 정렬 기준은 별칭 오름차순입니다. 🔃 아이콘을 클릭하면 페이지 데이터를 수동으로 새로 고침합니다.

{% hint style="info" %} **참고**
`postgres`, `template0`, `template1`은 시스템 기본 데이터베이스로, 목록에 표시되지만 선택하거나 삭제할 수 없습니다. {% endhint %}
...
\[취소\]를 클릭하거나 드로어를 닫으면 입력한 내용이 모두 초기화됩니다. \[생성\] 버튼을 클릭하기 전까지 변경사항이 저장되지 않습니다. {% endhint %}
...
{% hint style="warning" %} **주의**
삭제 작업은 되돌릴 수 없습니다. 데이터베이스에 포함된 모든 데이터와 객체가 영구적으로 삭제되며, 연결 중인 애플리케이션에 즉시 장애가 발생할 수 있습니다. {% endhint %}
...
{% hint style="info" %} **참고**
`postgres`, `template0`, `template1`은 라디오 버튼으로 선택할 수 없어 삭제할 수 없습니다. {% endhint %}
