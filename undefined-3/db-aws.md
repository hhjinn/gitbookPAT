OwlDB에서 데이터베이스를 운영하기 위해 데이터베이스를 생성합니다. 데이터베이스 생성(이하 프로비저닝)이 완료되면 OwlDB에서 제공하는 모든 기능을 사용할 수 있습니다.
...
데이터베이스 생성 마법사는 아래 순서대로 진행합니다. 페이지 우측 요약 영역에서는 각 단계에서 입력한 정보를 항상 확인할 수 있습니다.

 1. **OwlDB 콘솔 화면** > **대시보드** 메뉴로 이동합니다.
 2. **생성** 버튼을 클릭합니다.
 3. **엔진 옵션** 단계에서 데이터베이스 엔진 및 라이선스 관련 정보를 선택합니다.
 4. **DR 구성** 단계에서 DR 사용 여부와 관련 옵션을 설정합니다.
 5. **AZ 구성** 단계에서 가용 영역을 설정합니다.
 6. **인스턴스 구성** 단계에서 인스턴스 및 볼륨 정보를 입력합니다.
 7. **데이터베이스 구성** 단계에서 데이터베이스 정보를 입력합니다.
 8. **구성 정보 확인** 단계에서 전체 생성 정보를 검토합니다.
 9. License Option에 따라 버튼을 클릭하여 프로비저닝을 시작합니다.
    * **LI**: **생성** 버튼 클릭
    * **BYOL**: **라이선스 등록** 버튼 클릭 (라이선스 등록 절차는 [BYOL 라이선스 등록](#byol-%EB%9D%BC%EC%9D%B4%EC%84%A0%EC%8A%A4-%EB%93%B1%EB%A1%9D) 참고)
...
| Database Engine Type\* | 사용할 데이터베이스 엔진<br>- **Tibero** |
...
| Node Count\* | 클러스터를 구성할 노드 수<br>- Single 1개 (고정)<br>- TAC 2\~4개 중 선택 |
...
| 항목  | 설명  |
|-----|-----|
| Enable DR\* | DR 구성 사용 여부<br>- **DR 사용**: 다른 가용 영역에 Standby(Replica) 추가<br>- 장애 발생 시 자동 페일오버<br>- **DR 미사용**: Standby(Replica) 미추가<br>- 장애 발생 시 자동 페일오버 미제공 |
...
| Standby DB Availability Zone(AZ)\* | Standby DB의 가용 영역<br>- 기본값: OwlDB와 같은 가용 영역에 배치<br>- 이후 다른 영역에 자동 배치 |
...
|     | Redo Log Volume Size (disabled) | Redo log를 저장할 볼륨의 크기<br>- 입력한 Redo Log File Size(GB)에 따라 자동 계산 |
...
| Temporary Tablespace Data File Size (GB) | 대용량 연산에 사용되는 Temporary Tablespace(임시 테이블스페이스) 데이터 파일 크기 |
