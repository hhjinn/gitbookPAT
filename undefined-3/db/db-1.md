...
| 항목                   | 설명                                                                                                                                                         |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Service Name\*       | <p>데이터베이스 서비스를 식별하기 위한 이름<br>- 6~30자의 영문 대소문자(a-z, A-Z), 숫자(0-9), 하이픈(-)만 사용 가능<br>- 미입력 시<code>owldb-001</code>과 같은 형태로 자동 생성</p>                         |
| Database Engine Type | <p>사용할 데이터베이스 엔진<br>-<strong>Tibero</strong>: 다중화 구성으로 안정적인 서비스 운영 및 DB 확장이 가능한 RDBMS<br>-<strong>OpenSQL</strong> : Open Source 기반 고객 맞춤형 DBMS 기술 플랫폼</p> |
| Topology             | <p>데이터베이스 구조를 결정할 토폴로지 유형<br>-<strong>Tibero</strong>: Single, TAC<br>-<strong>OpenSQL</strong> : Single, HA</p>                                           |
| Node Count           | <p>클러스터 구성 노드 수<br>-<strong>Tibero</strong>Single : 1 / TAC : 2~8<br>-<strong>OpenSQL</strong> Single : 1 / HA : 2~3</p>                                   |
...
| Failover Automation Level\* | <p>자동 장애 조치 단계<br>-<strong>0단계 : 수동</strong><br>-<strong>1단계 : 자동 장애 조치</strong><br>-<strong>2단계 : 자동 구성 복구 (On-Premise 미지원)</strong><br>-<strong>3단계 : 완전 자동화</strong></p>                                                                                                                                | <p>- Tibero Single : 0, 1, 3단계 지원<br>- Tibero TAC : 0, 1단계 지원<br>- OpenSQL : 0, 3단계 지원</p> |
| {Standby/Replica} Count     | Standby/Replica DB 개수                                                                                                                                                                                                                                                                                      | -                                                                                          |
| Standby Mode                | <p>Standby Mode 옵션<br>-<strong>Recovery</strong><br>-<strong>Read Only</strong></p>                                                                                                                                                                                                                        | Tibero에서만 입력                                                                               |
| Log Replication Type        | <p>Primary(Leader)에서 Standby(Replica)로의 로그 전송 방식<br>-<strong>LGWR ASYNC</strong>(Tibero) : 트랜잭션이 발생하면 실시간으로 생성되는 Redo log를 전송하는 복제 모드<br>-<strong>ARCH ASYNC</strong>(Tibero) : 로그 스위치 이후, 아카이브 로그 파일이 생성되면 해당 파일을 모아서 전송하는 복제 모드<br>-<strong>ASYNC</strong>(OpenSQL) : 복제 연결을 통해 데이터를 비동기로 전송하는 복제 모드</p> | -                                                                                          |
...
| Service IP\*   | OwlDB와 노드간 통신에 사용할 IP를 직접 입력 또는 선택    | NAT를 사용하는 경우 NAT IP를 입력              |
| Service Port\* | OwlDB와 데이터베이스 서버간 통신에 사용할 Port를 직접 입력 | 포트 포워딩을 사용하는 경우 외부 Port를 입력          |
| Backup Path\*  | Backup Path 입력                        | <p>파일 시스템 경로만 입력 가능,<br>공유 볼륨 사용</p> |
...
| Hostname\*                  | DB 노드에 매핑할 호스트를 선택                                                     | -                           |
| Service IP\*                | OwlDB와 노드간 통신에 사용할 IP를 직접 입력 또는 선택                                     | NAT를 사용하는 경우 NAT IP를 입력     |
| Service Port\*              | <p>OwlDB와 데이터베이스 서버간 통신에 사용할 Port를 직접 입력<br>기본값 :<code>5432</code></p> | 포트 포워딩을 사용하는 경우 외부 Port를 입력 |
...
| VIP                                      | <p>데이터베이스 가상 IP<br>VIP를 사용하지 않는 경우<code>-</code>로 표시</p>                              |
...
| Redo Log File Size (MB)                  | <p>Redo 로그 파일 크기<br>탐색 과정에서 값을 확인할 수 없어 빈 값으로 표시되며 수정할 수 없음</p>                       |
| System Data File Size (MB)               | <p>시스템 테이블 및 주요 메타 데이터를 저장할 데이터 파일의 크기<br>탐색 과정에서 값을 확인할 수 없어 빈 값으로 표시되며 수정할 수 없음</p> |
| Syssub Data File Size (MB)               | <p>시스템 운영 관련 데이터 저장을 위한 서브 데이터 파일 크기<br>탐색 과정에서 값을 확인할 수 없어 빈 값으로 표시되며 수정할 수 없음</p>   |
| User Tablespace Data File Size (MB)      | <p>사용자 데이터를 저장할 테이블 스페이스 데이터 파일 크기<br>탐색 과정에서 값을 확인할 수 없어 빈 값으로 표시되며 수정할 수 없음</p>     |
| Temporary Tablespace Data File Size (MB) | <p>대용량 연산에 사용되는 임시 테이블스페이스 데이터 파일 크기<br>탐색 과정에서 값을 확인할 수 없어 빈 값으로 표시되며 수정할 수 없음</p>   |
| Undo Tablespace Data File Size (MB)      | <p>Undo 테이블스페이스 크기<br>탐색 과정에서 값을 확인할 수 없어 빈 값으로 표시되며 수정할 수 없음</p>                     |
|                                          |                                                                                       |
{% endtab %}
...
| Connection Pooler Port\* | <p>커넥션 풀이 클라이언트 연결을 수신하는 포트<br>범위 : 1024~65535</p>          |
|                          |                                                             |
{% endtab %}
...
\*_**표기는 필수 입력 항목을 의미합니다. 이외의 항목들은 탐색을 통해 수집된 정보가 자동으로 입력되어 있습니다.**_
