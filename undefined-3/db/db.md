변경 감지는 OwlDB 외부에서 데이터베이스 구성이 변경된 경우, 이를 탐지하여 OwlDB에 반영하는 기능입니다.

대시보드 상단의 **탐색** 버튼을 클릭하면, 변경이 감지된 DB가 있을 경우 대시보드에 표시됩니다. **등록 DB**에 한해서만 변경 감지 기능이 제공됩니다. 설치 DB는 변경 감지 대상에서 제외됩니다.

---

## 변경 유형

변경 감지는 아래 두 가지 유형으로 구분됩니다.

| 유형  | 설명  | 지원 엔진 |
|-----|-----|-------|
| 상태 변경 | DB 구성은 그대로이나 노드의 역할이 변경된 경우<br>(예: Failover 발생, Primary ↔ Standby 역할 전환) | Tibero |
| 스펙 변경 | OwlDB 외부에서 DB 구성이 변경된 경우 (예: Primary 또는 Standby 노드 Scale In/Out) | Tibero, OpenSQL |

{% hint style="info" %}
**참고**

* 상태 변경과 스펙 변경이 동시에 감지된 경우, 상태 변경이 우선 처리되며 대시보드에는 해당 DB가 상태 변경 DB로만 표시됩니다.
* 토폴로지 변경에 대해서는 변경 감지를 지원하지 않습니다.
* OpenSQL은 노드 역할 변경(상태 변경)을 Patroni가 수행하며, OwlDB가 감지 결과를 자동으로 반영하므로 사용자 조치 화면을 제공하지 않습니다. 자동 반영된 역할 변경은 전환 이력에 Failover로 기록되며, 스펙 변경이 함께 감지된 경우 대시보드에는 **스펙 변경** 버튼만 활성화됩니다.
{% endhint %}

---

## 스펙 변경된 DB

OwlDB 외부에서 수동으로 Scale In/Out이 발생한 경우, 탐색 후 대시보드에 **스펙 변경** 버튼이 활성화된 카드뷰로 표시됩니다.

* Scale In된 인스턴스는 카드뷰에서 표시되지 않습니다.
* Scale Out된 인스턴스는 해당 역할 목록 하단에 추가되어 표시됩니다.

### 반영 방법

1. 대시보드에서 스펙 변경이 감지된 DB를 선택합니다.
2. **스펙 변경** 버튼을 클릭하여 스펙 변경 페이지로 이동합니다.
3. 변경된 스펙 정보를 확인 및 추가 정보를 입력하여 OwlDB에 반영합니다.

---

## 상태 변경된 DB

OwlDB 외부에서 수동으로 역할 전환 또는 Failover가 발생한 경우, 탐색 후 대시보드에 **상태 변경** 버튼이 활성화된 카드뷰로 표시됩니다. 상태 변경 조치는 Tibero DB에서만 제공됩니다.

카드뷰에는 상태 변경이 적용되기 전까지 OwlDB에 저장된 기존 상태가 그대로 표시되며, 상태 변경 버튼을 클릭하면 변경 전후를 비교하는 모달 창이 로드됩니다.

* 왼쪽 카드뷰 : 기존 상태 (OwlDB에 저장된 상태)
* 오른쪽 카드뷰 : 탐색 후 감지된 현재 상태

{% hint style="info" %}
**참고**

상태 변경이 감지된 경우, 인스턴스의 Health 정보만 조회되며 CPU, Memory, 활성 세션 등 상세 정보는 표시되지 않습니다.
{% endhint %}

### 반영 방법

상태 변경만 감지된 경우와 상태 변경 + 스펙 변경이 동시에 감지된 경우로 나뉩니다.

**상태 변경만 감지된 경우**

1. 대시보드에서 상태 변경이 감지된 DB를 선택합니다.
2. **상태 변경** 버튼을 클릭합니다.
3. 모달 창에서 변경 전후 상태를 확인합니다.
4. 아래 중 원하는 동작을 선택합니다.

* **적용** : 상태 변경 내역을 OwlDB에 반영하고 모달을 닫습니다.
* **취소** : 변경 사항을 반영하지 않고 모달을 닫습니다.

**상태 변경 + 스펙 변경이 동시에 감지된 경우**

1. 대시보드에서 상태 변경이 감지된 DB를 선택합니다.
2. **상태 변경** 버튼을 클릭합니다.
3. 모달 창에서 변경 전후 상태를 확인합니다.
4. 아래 중 원하는 동작을 선택합니다.

* **적용** : 상태 변경만 OwlDB에 반영하고 대시보드로 돌아갑니다.
* **취소** : 변경 사항을 반영하지 않고 모달을 닫습니다.
* **스펙 변경으로 이동** : 상태 변경을 반영한 후, 스펙 변경 페이지로 이동하여 스펙 변경도 함께 반영합니다.

{% hint style="info" %}
**참고**

**적용**을 선택하여 상태 변경만 반영한 경우, 대시보드에는 스펙 변경 버튼이 표시되지 않습니다. 반영하지 않은 스펙 변경 사항이 남아있다면 다시 **탐색**을 실행할 때 해당 DB가 스펙 변경 대상으로 다시 표시됩니다.
{% endhint %}
