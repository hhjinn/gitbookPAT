...
- OwlDB에서 지원하는 데이터베이스 엔진 및 인스턴스 타입에 대한 내용은 '[서비스 개요](#XDj4D6jZeLIG3hl9e9W4)' 페이지를 참고하시기 바랍니다.
{% endhint %}
...
- Azure 환경에서는 라이선스 옵션이 BYOL(Bring Your Own License)로 고정되어 있어, 데이터베이스 생성을 완료하려면 보유한 라이선스 파일을 등록해야 합니다. 자세한 내용은 [BYOL 라이선스 등록](#byol-라이선스-등록)을 참고하시기 바랍니다.
{% endhint %}
...
| 항목 | 설명 |
| --- | --- |
| DB Service Name* | 데이터베이스 서비스를 식별하기 위한 이름<br>- OwlDB 계정 내 중복 사용 불가<br>- 6~30자 이내, 영어 대소문자(a-z, A-Z), 숫자(0-9), 하이픈(-)만 사용 가능, 공백 사용 불가 |
...
|   | Redo Log Disk Size (disabled) | Redo log를 저장할 디스크의 크기<br>- 입력한 Redo Log File Size(GB)에 따라 자동 계산 |
