...
| 타입 | 설명 |
| --- | --- |
| Provisioning | 신규 리소스 생성 중 (Cloud 환경에서 DB 서비스 생성 시 해당) |
...
| Stopping | Running 상태에서 Stopped로 전환 중 (Cloud 환경에서는 트랜잭션 롤백 및 프로세스 정지 후 VM 중지 전환, On-Premise 환경에서는 DB 전체 종료 수행) |
...
| 엔진 | 표시 | 상태 | 메시지 | 설명 |
| --- | --- | --- | --- | --- |
| Tibero | 🟢 Available | normal | - | 노드 정상 |
| Tibero | 🟡 Limited | normal | Issue: CMT Inactive | 클러스터 관리 도구(CMT) 비정상 |
| Tibero | 🟡 Limited | normal | Issue: Mount Mode | 노드가 Mount 모드로 기동됨 |
| Tibero | 🔴 Unavailable | normal | Issue: Nomount Mode | 노드가 Nomount 모드로 기동됨 |
| Tibero | 🔴 Unavailable | normal | Issue: DB Down | 노드의 DB Down |
| Tibero | 🔴 Unavailable | normal | Issue: Agent Disconnect | 노드가 위치한 VM과 Agent 연결 끊김 |
| Tibero | 🔴 Unavailable | normal | Issue: VM Down | 노드가 위치한 VM Down |
| Tibero | ⚫ Retired | normal | Issue: Failovered Primary | Failover 수행 과정에서 (구)Primary 비정상 종료 |
| OpenSQL | 🟢 Available | normal | - | 노드 정상 |
| OpenSQL | 🟡 Limited | normal | Issue: etcd Inactive | etcd 비정상 |
| OpenSQL | 🟡 Limited | normal | Issue: OpenProxy Inactive | OpenProxy 비정상 |
| OpenSQL | 🔴 Unavailable | normal | Issue: DB Down | Patroni 비정상으로 DB Down |
| OpenSQL | 🔴 Unavailable | normal | Issue: Agent Disconnect | 노드가 위치한 VM과 Agent 연결 끊김 |
...
전체 데이터베이스와 하위 인스턴스 목록을 확인합니다. '[전체 상태 요약 정보 확인](#전체-상태-요약-정보-확인)'에서 선택한 상태값에 따라 표시되는 데이터베이스 및 인스턴스 목록이 달라집니다.
...
DB 서비스에 대한 다양한 관리 작업을 수행할 수 있습니다. 자세한 내용은 작업 페이지에서 확인할 수 있습니다.
{% endhint %}
...
| 항목 | 설명 |
| --- | --- |
| DB 서비스 정보 | 데이터베이스의 주요 정보를 카드 상단에 요약 표시<br>표시 항목<br>- DB Type Logo<br>- DB Service Name<br>- Status<br>- DB Type<br>- DB version (OpenSQL의 경우 OpenSQL 버전과 PostgreSQL 버전 동시 표시)<br>- Topology<br>- Eventlog : I / W / E (각각 Info / Warning / Error) |
...
| --- | --- | --- | --- | --- | --- |
| **이름** | 이름 표시<br>- DB 서비스 이름 클릭 시 '[서비스 메타 정보 조회](https://github.com/hhjinn/gitbookPAT/tree/dori/TgpIeVAAe6TUtfrjaD2G/doc/5d7daeb2-e4f4-4409-aa74-dcaf1d65ab46/README.md)' 페이지로 이동<br>- 인스턴스 별칭 클릭 시 '[인스턴스 관리](https://github.com/hhjinn/gitbookPAT/tree/dori/TgpIeVAAe6TUtfrjaD2G/doc/dF57s45IXBUgU7RX1UvL/README.md)' 페이지로 이동 | DB 서비스 이름 | 인스턴스 별칭 | O | O |
| **생성일** | DB 서비스 생성일을 `yyyy.mm.dd HH:mm:ss` 형식으로 표시 | DB 서비스 생성일 | X | X | X |
| **상태** | Status 또는 Health 표시 | - Creating<br>- Running(n/m)<br>- Updating(n/m)<br>- Degraded(n/m)<br>- Down(n/m)<br>- Starting<br>- Stopping<br>- Stopped<br>- Terminating<br>- Deploying (on-premise)<br>- Registering (on-premise)<br>- Unregistering (on-premise) | - 🟢 (available)<br>- 🟡 (limited)<br>- 🔵 (in progress)<br>- 🔴 (unavailable) | O | O |
...
| **vCPU** | CPU 수와 사용량을 바 차트로 표시 | O | O | O |   |
| **Memory** | Memory와 사용량을 바 차트로 표시 | O | O | O | X |
| **활성세션** | 현재 활성화된 세션의 개수를 바 차트로 표시<br>- Primary / Standby(RO) / Leader / Replica 에 한하여 표시<br>- Standby(Recovery) : `-`으로 표시 | O | O | O | X |
...
| 항목 | 설명 |
| --- | --- |
| DB Service Info | DB 서비스 기본 정보 표시 |
| Node Info | 노드별 상태 및 리소스 정보 표시 |
| Cluster Info | Cluster 정보 및 Connection Health 표시 |
