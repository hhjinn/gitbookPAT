...
| 항목                 | 설명                                                                                                                                                            |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Service Name\\\*   | <p>DB 서비스를 식별하기 위한 이름</p><ul><li>6\~30자의 영어 대소문자, 숫자, 하이픈(-)만 입력 가능</li><li>OwlDB 계정 내에서 중복 생성 불가</li><li>기본값 :<code>owldb-001</code>부터 순차 부여</li></ul>       |
| DB Engine Type\\\* | <p>사용할 데이터베이스 엔진</p><ul><li><strong>Tibero</strong>: 다중화 구성으로 안정적인 서비스 운영 및 DB 확장이 가능한 RDBMS</li><li><strong>OpenSQL</strong> : Open Source 기반 DBMS</li></ul> |
| Topology\\\*       | <ul><li><strong>Tibero</strong>: Single, TAC</li><li><strong>OpenSQL</strong> : Single, HA</li></ul>                                                          |
| Node Count\\\*     | <ul><li><strong>Tibero</strong>: Single(1, 고정), TAC(2\~4 중 선택)</li><li><strong>OpenSQL</strong> : Single, HA(1, 고정)</li></ul>                                 |
| PostgreSQL Version | <p>OpenSQL에서 사용할 PostgreSQL 버전(Tibero는 해당 없음)</p><ul><li>기본값 :<strong>17.9</strong></li></ul>                                                                 |
...
| Failover Automation Level\\\* | <p><a href="db-1.md#undefined">자동 장애 조치 단계</a></p><ul><li>0단계 : 수동</li><li>1단계 : 자동 장애 조치</li><li>2단계 : 자동 구성 복구 (OwlDB v1.3 미지원)</li><li>3단계 : 완전 자동화</li><li>Single : 0, 1, 3단계 지원</li><li>TAC : 0, 1단계 지원</li></ul> |
| Standby Count\\\*             | Standby DB 개수(표준 아키텍처 기준 최대 1개로 고정)                                                                                                                                                                                    |
| Standby Mode\\\*              | <p>Standby Mode 옵션</p><ul><li><strong>Recovery</strong></li><li><strong>Read Only</strong></li></ul>                                                                                                                   |
| Log Replication Type          | <p>Primary에서 Standby로의 로그 전송 방식</p><ul><li><strong>LGWR ASYNC</strong>: 트랜잭션이 발생하면 실시간으로 생성되는 Redo log를 전송하는 복제 모드</li><li><strong>ARCH ASYNC</strong> : 로그 스위치 이후, 아카이브 로그 파일이 생성되면 해당 파일을 모아서 전송하는 복제 모드</li></ul>   |
|                               |                                                                                                                                                                                                                        |
|                               |                                                                                                                                                                                                                        |
| 항목                            | 설명                                                                                                                                                                                                                     |
| ---                           | ---                                                                                                                                                                                                                    |
| Enable DR\\\*                 | Single은 DR 미사용, HA는 DR 사용으로 자동 결정되며 수정 불가                                                                                                                                                                              |
| Failover Automation Level\\\* | <p><a href="db-1.md#undefined">자동 장애 조치 단계</a></p><ul><li>0단계 : 수동</li><li>2단계 : 자동 구성 복구 (OwlDB v1.3 미지원)</li><li>3단계 : 완전 자동화</li><li>0, 3단계 지원(기본값 3단계)</li></ul>                                                   |
...
| VIP\\\*                                  | VIP 사용 여부 선택                                                                                      |
| Primary Node #N Vip                      | 데이터베이스 가상 IP\n(VIP 사용 선택시 활성화)                                                                    |
...
| WAL File Size (MB)                       | WAL 파일 크기\n탐색 과정에서 값을 확인할 수 없어 빈 값으로 표시되며 수정할 수 없음                                                |
| Connection Pooler Port                   | <p>OpenSQL에서 커넥션 풀이 클라이언트 연결을 수신하는 포트</p><ul><li>기본값 : 6432</li><li>입력 범위 : 1024\~65535</li></ul> |
