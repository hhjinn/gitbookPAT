...
<table><thead><tr><th width="249">항목</th><th>설명</th></tr></thead><tbody><tr><td>Service Name*</td><td>DB 서비스를 식별하기 위한 이름<br>- 6~30자의 영어 대소문자, 숫자, 하이픈(-)만 입력 가능<br>- OwlDB 계정 내에서 중복 생성 불가<br>- 기본값 :<code>owldb-001</code>부터 순차 부여</td></tr><tr><td>DB Engine Type*</td><td>사용할 데이터베이스 엔진<br>-<strong>Tibero</strong>: 다중화 구성으로 안정적인 서비스 운영 및 DB 확장이 가능한 RDBMS<br>-<strong>OpenSQL</strong> : Open Source 기반 DBMS</td></tr><tr><td>Topology*</td><td>- <strong>Tibero</strong>: Single, TAC<br>-<strong>OpenSQL</strong> : Single, HA</td></tr><tr><td>Node Count*</td><td>- <strong>Tibero</strong>: Single(1, 고정), TAC(2~4 중 선택)<br>-<strong>OpenSQL</strong> : Single, HA(1, 고정)</td></tr><tr><td>PostgreSQL Version</td><td>OpenSQL에서 사용할 PostgreSQL 버전(Tibero는 해당 없음)<br>- 기본값 :<strong>17.9</strong></td></tr></tbody></table>
...
{% tabs %}
{% tab title="Tibero" %}
<table><thead><tr><th width="245">항목</th><th>설명</th></tr></thead><tbody><tr><td>Enable DR*</td><td>DR 구성 사용 여부(직접 선택 가능)</td></tr><tr><td>Failover Automation Level*</td><td><a href="db-1.md#undefined">자동 장애 조치 단계</a><br>- 0단계 : 수동<br>- 1단계 : 자동 장애 조치<br>- 2단계 : 자동 구성 복구 (OwlDB v1.3 미지원)<br>- 3단계 : 완전 자동화<br>- Single : 0, 1, 3단계 지원<br>- TAC : 0, 1단계 지원</td></tr><tr><td>Standby Count*</td><td>Standby DB 개수(표준 아키텍처 기준 최대 1개로 고정)</td></tr><tr><td>Standby Mode*</td><td>Standby Mode 옵션<br>-<strong>Recovery</strong><br>-<strong>Read Only</strong></td></tr><tr><td>Log Replication Type</td><td>Primary에서 Standby로의 로그 전송 방식<br>-<strong>LGWR ASYNC</strong>: 트랜잭션이 발생하면 실시간으로 생성되는 Redo log를 전송하는 복제 모드<br>-<strong>ARCH ASYNC</strong> : 로그 스위치 이후, 아카이브 로그 파일이 생성되면 해당 파일을 모아서 전송하는 복제 모드</td></tr></tbody></table>
{% endtab %}

{% tab title="OpenSQL" %}
<table><thead><tr><th width="256">항목</th><th>설명</th></tr></thead><tbody><tr><td>Enable DR*</td><td>Single은 DR 미사용, HA는 DR 사용으로 자동 결정되며 수정 불가</td></tr><tr><td>Failover Automation Level*</td><td><a href="db-1.md#undefined">자동 장애 조치 단계</a><br>- 0단계 : 수동<br>- 2단계 : 자동 구성 복구 (OwlDB v1.3 미지원)<br>- 3단계 : 완전 자동화<br>- 0, 3단계 지원(기본값 3단계)</td></tr><tr><td>Standby Count*</td><td>Replica DB 개수(표준 아키텍처 기준 최대 1개로 고정)</td></tr><tr><td>Log Replication Type</td><td>ASYNC 방식으로 고정되어 수정 불가</td></tr></tbody></table>
{% endtab %}
{% endtabs %}
...
| WAL File Size (MB)     | <p>WAL 파일 크기<br>탐색 과정에서 값을 확인할 수 없어 빈 값으로 표시되며 수정할 수 없음</p>                       |
| Connection Pooler Port | <p>OpenSQL에서 커넥션 풀이 클라이언트 연결을 수신하는 포트<br>- 기본값 : 6432<br>- 입력 범위 : 1024~65535</p> |
...
{% tabs %}
{% tab title="Tibero" %}
<table><thead><tr><th width="187">주요 단계</th><th>세부 단계</th></tr></thead><tbody><tr><td>설치 필수 조건 검증</td><td>1. sudo 권한 검증<br>2. 필수 파일 검증<br>3. parameter config 검증</td></tr><tr><td>인프라 설정</td><td>1. Kernel 환경 설정<br>2. 필수 package 설치<br>3. Disk udev rule 설정<br>4. Volume mount<br>5. Disk attach (instance)</td></tr><tr><td>Tibero 설정</td><td>1. Tibero instance Tip file &#x26; DSN file 생성<br>2. cm 순차설치로 인한 volume 설정 변경<br>3. cm 순차설치로 인한 volume 대기<br>4. Tip covert(primary &#x3C;→ standby)<br>5. Standby 설치 완료 tag 설정 변경<br>6. Standby 설치 완료 tag 대기<br>7. RMGR 백업 및 전송<br>8. RMGR 백업 대기</td></tr><tr><td>DB 설치</td><td>1. CM gen(resource 등록 및 실행)<br>2. DB create(TAS도 포함됨 토폴로지에 따라)<br>3. Disk snapshot 생성<br>4. wait Disk snapshot in Standby node<br>5. CM Fence on(reboot)<br>6. cm complete(service up)<br>7. wait cm service<br>8. recovery RMGR</td></tr><tr><td>Tibero 상태 확인</td><td>1. Tb probe</td></tr></tbody></table>
{% endtab %}
