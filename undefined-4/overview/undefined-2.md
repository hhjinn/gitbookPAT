...
- 대시보드에서 다음 경로를 통해 인스턴스 관리 페이지로 이동합니다. 리스트뷰: **데이터베이스 별칭 옆 화살표 아이콘 > 인스턴스 별칭** 클릭 카드뷰: 데이터베이스 카드의 **인스턴스 별칭** 클릭
...
| Memory | 프로비저닝된 메모리 대비 사용량 바 차트 |
| 활성 세션 | 활성화된 세션 수 바 차트 |
| Total Volume | root, data, WAL log, archive log 볼륨 사용량 총합 |
| Volume | data, WAL log, archive log 볼륨 사용량 총합 |
| Root Volume | root 볼륨 사용량 |
| Current Log | 가장 최근 WAL log 식별값(LSN, Standby 구성 시에만 표시) |
{% endtab %}
{% endtabs %}
...
| log last received | Primary로부터 최근 수신한 WAL log 식별값(LSN 값) |
| log last applied | Standby에 최근 적용된 WAL log 식별값(LSN 값) |
| Replication Lag(초) | Primary DB와의 복제 지연 시간 |
{% endtab %}
{% endtabs %}
...
| log last received | Primary로부터 수신한 최근 WAL log(LSN) | Standby |
| log last applied | Standby에 적용된 최근 WAL log(LSN) | Standby |
| Replication Lag(초) | Primary와의 복제 지연 시간 | Standby |
{% endtab %}
{% endtabs %}
