...
지원하는 장애 조치 자동화 레벨은 [**장애 조치 자동화 문서**](undefined-1.md#GDPaQdLZBmgq4vRqB2Sz)를 참고하십시오.
{% endhint %}
...
| Failover Automation Level | <p>0단계(수동), 1단계(자동 장애 조치), 3단계(완전 자동화) 지원.<br>2단계(자동 구성 복구)는 OwlDB v1.3에서 미지원</p> |
...
| Log Replication Type | <p>Standby 노드의 로그 복제 방식을 선택합니다. (Tibero 해당)<br>-<strong>LGWR ASYNC</strong>: 트랜잭션 발생 시 실시간으로 생성되는 Redo log를 전송하는 복제 모드<br>-<strong>ARCH ASYNC</strong> : 로그 스위치 이후 생성된 아카이브 로그 파일을 모아서 전송하는 복제 모드</p> |
...
**완료** 클릭 시 라이선스 파일 유무, CP Max Core 수, 라이선스 옵션 일치 여부를 확인하며, 다음의 경우 스펙 변경이 진행되지 않고 오류 메시지가 표시됩니다.
