...
각 파라미터는 이름, 데이터 형식, 기본값, 현재값, Config값, 동적 파라미터 여부를 표시합니다. 동적 파라미터는 데이터베이스 재시작 없이 즉시 적용할 수 있으며, 정적 파라미터는 수정 후 재시작이 필요합니다. Tibero는 재시작 없이 변경 내용을 임시 반영하는 임시 적용도 지원합니다.
...
**참고** 파란색 하이라이팅으로 강조된 파라미터 이름은 현재 수정 중인 값입니다.

1. **저장** 버튼을 클릭하고 수정 사항 미리보기를 확인합니다.
2. **적용** 또는 **임시 적용**을 클릭해 변경 사항을 저장합니다.
