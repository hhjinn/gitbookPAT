...
<table><thead><tr><th width="255">항목</th><th>설명</th></tr></thead><tbody><tr><td>이름</td><td>파라미터 이름</td></tr><tr><td>데이터 형식</td><td>파라미터의 데이터 타입</td></tr><tr><td>기본값</td><td>사용자가 별도로 설정하지 않았을 때 적용되는 값</td></tr><tr><td>현재값</td><td>현재 DB에 적용된 값</td></tr><tr><td>Config 값</td><td>DB 재시작 시 반영될 값</td></tr><tr><td>동적 파라미터</td><td>- <strong>예</strong>: 재시작 없이 즉시 반영 가능<br>-<strong>아니요(재시작 필요)</strong>: 반영하려면 DB 재시작 필요</td></tr></tbody></table>
...
3. **수정** 버튼을 클릭합니다.
4. 파라미터 값을 수정합니다.
5. 특정 파라미터를 원래 값으로 되돌리려면 복원합니다. 파라미터를 선택합니다. **기본값 복원** 또는 **현재값 복원** 버튼을 클릭합니다.
6. **저장** 버튼을 클릭합니다.
7. 수정 사항 미리보기를 확인합니다.
8. **적용** 또는 **임시 적용**을 클릭해 변경 사항을 저장합니다.
...
<table><thead><tr><th width="195">수정 대상</th><th>수정 가능 조건</th></tr></thead><tbody><tr><td>현재값</td><td>인스턴스 상태가 정상(<code>Available</code> 또는 <code>Limited</code>)일 때</td></tr><tr><td>Config 값</td><td>- 인스턴스 상태 <code>Unavailable</code>(DB Down/Nomount)일 때<br>- Config 값 없는 파라미터는 수정 불가</td></tr></tbody></table>
...
<table><thead><tr><th width="253">수정한 파라미터 유형</th><th>저장 방식</th></tr></thead><tbody><tr><td>동적 + 정적 파라미터 혼합</td><td>- <strong>적용</strong>: DB 재시작 후 변경 사항 반영 (연결 세션 종료, 수 분 소요)</td></tr><tr><td>동적 파라미터만</td><td>- <strong>적용</strong>: 재시작 없이 현재값에 즉시 반영<br>-<strong>임시 적용</strong>: 재시작 없이 현재값에 반영, DB 재시작 시 기존 Config 값으로 복원</td></tr></tbody></table>
...
<table><thead><tr><th width="156">템플릿</th><th>설명</th></tr></thead><tbody><tr><td>OLAP</td><td>대규모 데이터 분석과 복잡한 쿼리에 최적화된 템플릿</td></tr><tr><td>OLTP</td><td>빠른 처리 속도와 높은 트랜잭션 빈도를 처리하기 위해 설계된 템플릿</td></tr></tbody></table>
