...
| 항목       | 설명                                                                                     |
| -------- | -------------------------------------------------------------------------------------- |
| Endpoint | <ul><li>서비스 대표 접속 주소</li><li>Single: Private IP 표시</li><li>HA·TAC·DR: VIP 표시</li></ul> |
...
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 별칭         | 인스턴스 별칭                                                                                                                                                                      |
| 역할         | <ul><li><p>Tibero</p><ul><li>Primary</li><li>Standby(Recovery)</li><li>Standby(Read Only)</li></ul></li><li><p>OpenSQL</p><ul><li>Leader</li><li>Replica</li></ul></li></ul> |
| VIP        | 인스턴스 접속용 VIP 주소                                                                                                                                                              |
...
| 항목          | 설명                        | 입력 규칙                                                                                                                                                                                                                                         |
| ----------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Priority    | 규칙 적용 순서 (숫자가 작을수록 우선 적용) | <ul><li>미입력 시 마지막 순서로 추가</li><li>시스템 고정 규칙 번호 이후부터 입력 가능</li></ul>                                                                                                                                                                            |
| Type \*     | 연결 유형                     | <ul><li><code>local</code> / <code>host</code> / <code>hostssl</code> / <code>hostnossl</code>중 선택</li><li>기본값<code>host</code></li></ul>                                                                                                     |
| Database \* | 규칙을 적용할 데이터베이스            | <ul><li>데이터베이스 목록에서 하나 이상 선택 또는 특수 키워드(<code>all</code>, <code>sameuser</code>, <code>samerole</code>) 중 하나 선택</li><li>특수 키워드와 데이터베이스 목록 동시 선택 불가</li></ul>                                                                                   |
| User \*     | 규칙을 적용할 사용자               | 사용자 목록에서 하나 이상 선택 또는 `all` 선택                                                                                                                                                                                                                 |
| Address \*  | 접근을 허용할 클라이언트 주소          | <ul><li>CIDR 또는 Hostname 직접 입력 또는 특수 키워드(<code>all</code>, <code>samehost</code>, <code>samenet</code>) 선택</li><li>Type이<code>local</code>이면 비활성화</li><li>단일 IP 입력 시 자동 CIDR 형식 변환 (IPv4:<code>/32</code>, IPv6: <code>/128</code>)</li></ul> |
| Method \*   | 인증 방식                     | <ul><li>드롭다운에서 선택</li><li>기본값<code>scram-sha-256</code></li></ul>                                                                                                                                                                             |
| Auth Option | Method에 대한 세부 인증 옵션       | <ul><li>Method에 따라 입력 방식 상이</li><li><code>trust</code> 또는 <code>reject</code>선택 시 비활성화</li><li><code>scram-sha-256</code> 또는 <code>md5</code>선택 시 드롭다운 선택</li><li>그 외 Method는<code>key=value</code> 형식 입력</li></ul>                           |
...
| Servers \*       | 접속할 DB 서버                           | <ul><li>드롭다운에서 1개 이상 선택</li><li>인스턴스 역할(Role)·별칭(Instance Alias) 표시</li></ul> |
...
| Servers \*       | 접속할 DB 서버                       | <ul><li>드롭다운에서 1개 이상 선택</li><li>인스턴스 역할(Role)·별칭(Instance Alias)·Health 표시</li><li>Health는 참고용, 서버 선택에 영향 없음</li></ul> |
| Use Patroni      | Patroni를 통한 Auto Failover 사용 여부 | 항상 활성화(변경 불가)                                                                                                          |
