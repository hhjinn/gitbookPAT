...
백업은 특정 시점의 데이터베이스 상태를 이미지로 저장합니다. Cloud 환경에서는 단일 백업 이미지 단위로 목록을 표시합니다. 백업은 스케줄러를 통해 자동 생성되거나 사용자가 수동으로 생성할 수 있으며, 완전 복구(Full Restore) 또는 특정 시점 복구(PITR)를 통해 원하는 시점으로 복구할 수 있습니다. 백업 저장 용량을 기준으로 별도 요금이 발생합니다.
...
| 복구 유형 | 설명 |
| --- | --- |
| Full Restore | 마지막 커밋 시점까지 전체 복구 |
...
3. 복구 날짜와 시각을 입력합니다.
4. **복구** 버튼을 클릭합니다.
5. 복구 내역 조회에서 진행 상태를 확인합니다.

{% hint style="warning" %}
**주의**

보관 복구 시, 즉시 데이터베이스가 복구되며 현재 저장된 모든 백업이 삭제됩니다. 데이터 크기에 따라 최대 3일 가량 소요될 수 있습니다.
{% endhint %}
