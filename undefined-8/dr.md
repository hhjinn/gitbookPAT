...
| 1단계 (**자동 장애 조치**) | ✓           | - TAC Scale-Out으로 노드 수만 복구<br>- 신규 Standby·Replica 미생성 → DR 미복구(`Degraded`) | Old Primary/Leader를 `retired` 처리 |
| 2단계 (**자동 구성 복구**) | ✓           | - TAC Scale-Out<br>- 신규 Standby·Replica 자동 생성 → 고가용성 구성 복구 | Old Primary/Leader `retired` 처리 |
| 3단계 (**완전 자동화**) | ✓           | - Old Primary·Leader 역동기화<br>- 신규 Standby·Replica 자동 생성 | 없음  |
