...
1. From the left menu, click **Monitoring > Instance Monitoring**.
2. In the DB Type toggle of the GNB, select **Tibero** or **OpenSQL**.
3. In the DB Select tree, select the Service and Instance to view. OpenSQL lets you select down to the Database level.
4. **(OpenSQL only)** As needed, click **Instance View** or **Database View**.
5. Select a tab (**Quick Insight** / **Load** / **I/O** / **Writes** / **All**) to view the desired metrics.
6. Click the ⚙️ icon to configure auto-refresh.
7. Click the 🔃 icon to manually refresh the data.

#### DB Type and Resource Selection

When you select **Tibero** or **OpenSQL** in the DB Type toggle of the GNB, the DB Select tree and matching metrics are displayed. When you switch the DB Type, DB Select resets to select-all.
...
| Writes        | Check for surges in bulk write operations and transaction rollbacks |
...
When OpenSQL is selected, the screen also displays switch buttons for **Instance View** and **Database View**.
