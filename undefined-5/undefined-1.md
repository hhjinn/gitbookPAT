...
3. 검색 조건을 선택합니다.
4. 검색어를 입력해 원하는 세션을 필터링합니다.
5. Elapsed Time 알림 설정(⚙️) 아이콘을 클릭합니다.
6. 임계값을 입력하고 저장합니다.
7. 세션 행을 클릭해 상세 정보 드로어를 엽니다.
8. 📋 아이콘을 클릭해 SQL 전문을 복사합니다.

테이블 상단에서 다음 기능을 이용합니다.

| 기능 | 설명 |
| --- | --- |
| 수동 새로고침(🔃) | 클릭 시 세션 데이터 즉시 갱신 |
| 자동 새로고침(⚙️) | 자동 갱신 여부와 주기 설정 |
| 검색 | 조건 선택 및 검색어 입력으로 세션 필터링 |
| 테이블 설정(⚙️) | 테이블에 표시할 컬럼 추가/숨김 |
...
| 경고* | 경고 수준 임계값 (초) | - `0.1` ~ `1000.0`<br>- 소수점 첫째 자리까지 허용 |
| 주의* | 주의 수준 임계값 (초) | - `0.1` ~ `999.9`<br>- 소수점 첫째 자리까지 허용<br>- 경고 수준보다 작아야 함 |
...
| Database Name | 세션이 연결된 데이터베이스 이름 | — | ✓ |
| State | 세션의 세부 실행 단계 | ✓ | ✓ |
