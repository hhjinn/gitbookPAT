...
| 39mUAdbrI82f | Le8ZKtHErDYZ |
| --- | --- |
| **항목** | **설명** |
| 이용 기간 | - 당월 1일부터 UTC 기준 조회 전일까지 기간 표시<br>- 표기: `YYYY.MM.DD ~ YYYY.MM.DD` |
| DB 서비스 개수 | - 조회 기간 내 이용한 DB 서비스 총개수<br>- DB 유형별(Tibero / OpenSQL) 개수 함께 표시 |
| 총 이용 요금 | - 조회 기간 내 이용한 DB 서비스 라이선스 요금 합계<br>- 달러($) 기준, 소수점 둘째 자리 반올림 |

## 이용 현황 목록

| RbfxJqME71mT | ii32sK2JD2Xv |
| --- | --- |
| **항목** | **설명** |
| DB 유형 | DB 서비스 유형 (`Tibero` / `OpenSQL`) |
| 이름 | 조회 기간 내 이용한 DB 서비스 이름<br>- 삭제된 서비스는 이름 앞 `(Terminated)` 표기<br>- 행 전체 빨간색 표시 |
