...
| 항목             | 설명                                                                        |
| -------------- | ------------------------------------------------------------------------- |
| Subscription   | <p><strong>Azure 구독 계정</strong><br>- 모든 리소스의 집합으로, 구독의 모든 리소스는 함께 청구됨</p> |
| Resource Group | <p><strong>Azure 리소스 그룹</strong><br>- 동일한 수명 주기, 권한 및 정책을 공유하는 리소스 모음</p> |
...
| 항목                      | 설명                                                                                                | 비고                                                      |
| ----------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| Region                  | OwlDB를 배포할 리전                                                                                     | [제공 리전 확인](마켓플레이스-구독-안내.md#undefined-1)                 |
...
| Public Subnet CIDR      | <p>지정한 VNet 내에서 사용할 Public Subnet의 CIDR 범위<br>* Public Subnet : 인터넷 게이트웨이를 통해 외부 통신이 가능한 네트워크</p> | VPC CIDR에 포함되는 범위여야 함                                   |
...
| SSH Public Key          | OwlDB 인스턴스에 SSH로 접근하기 위한 Key Pair 이름                                                              | <p>• SSH Key 로 미리 생성되어 있어야 함<br>• PEM 파일은 로컬에 보관 필요</p> |
| OwlDB Root Username     | OwlDB에 로그인하기 위한 기본 관리자 계정의 ID                                                                     | <p><strong>Default : admin</strong><br>설정 이후 변경 불가</p>  |
...
마켓플레이스에서 OwlDB 배포가 완료되면, [**OwlDB 배포 시 입력한 메일 계정**](마켓플레이스-구독-안내.md#id-2.-instance-details)으로 OwlDB 접속 주소와 계정 정보가 포함된 안내 메일이 발송됩니다. 해당 메일을 통해 OwlDB에 접속할 수 있습니다. 메일이 수신되지 않을 경우, [azure\_owldb\_support@tibero.com](mailto:azure_owldb_support@tibero.com)으로 문의 주시기 바랍니다.
