...
{% hint style="info" %}
**참고** BYOL 라이선스 모델을 사용 중이고 라이선스 만료일이 3개월 이내로 남은 경우, 화면 상단에 만료 예정 안내 배너가 표시됩니다. 배너의 **갱신 페이지로 이동** 링크를 클릭하면 라이선스 갱신 화면으로 이동할 수 있습니다.
{% endhint %}
...
{% hint style="info" %}
**참고** 일시가 표시되는 항목은 모두 데이터베이스 타임존을 기준으로 표시됩니다.
{% endhint %}
{% endtab %}
{% tab title="인스턴스" %}
...
{% hint style="info" %}
**참고** DR 구성을 사용하는 경우, Primary(Leader) DB와 Standby(Replica) DB를 구분하여 조회합니다.
{% endhint %}
{% endtab %}
{% tab title="설치 정보" %}
...
{% hint style="info" %}
**참고** 유휴 라이선스가 존재하는 경우, **재구축** 버튼을 통해 기존 라이선스 구성대로 인스턴스 생성 및 라이선스 할당을 수행할 수 있습니다.
{% endhint %}
{% endtab %}
