등록할 DB의 배포 파일을 배치하고 owlagent를 설치·기동하기까지의 환경 준비 절차를 설명합니다.
...
| 결과 | 판정 |
| --- | --- |
| 0 | `patroni.service`로 등록·기동되어 있음 → 점검 완료 |
| 0 이외 | 아래 두 가지가 구분되지 않으므로 b를 추가로 수행합니다* 다른 유닛명으로 기동된 경우* systemd 미등록인 경우 |
| b. 유닛명 확인 |   |
...
| 결과 | 판정 |
| --- | --- |
| 0::/system.slice/xxxxxxxx.service | 유닛명 불일치 → c를 추가로 수행합니다. |
