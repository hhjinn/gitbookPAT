...
| OPENSQL\_HOME       |                    | <p>- 이미 설정 시 입력 불필요<br>- 미설정 시 위에서 사용한 OPENSQL_HOME 입력</p> |
| DB\_LOG\_DIR        | PG 로그 경로           | 로그 미수집 시 입력 불필요                                            |
| DB\_LOG\_FILE\_GLOB | PG 로그 파일 형식        | 예: `postgresql*.log`                                       |
| PATRONI\_CONFIG     | patroni.yml 경로     |                                                            |
| PATRONI\_MEMBER     | patroni 멤버 이름      |                                                            |
