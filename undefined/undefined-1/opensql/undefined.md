...
OwlDB에서 관제하는 데이터베이스 서버에 해당하는 공통 준비 절차입니다. 디스크 요구사항, 네트워크 설정, 배포 파일 구성 등 구성 방식에 따른 다른 절차는 각각 [설치 DB 환경 준비 가이드](db.md)와 [등록 DB 환경 준비 가이드](db-1.md)를 참고합니다.
...
* SSH 포트: `22`
* 데이터베이스 노드
  * Node1 (Leader): `10.10.0.11`
  * Node2 (Replica): `10.10.0.12`
  * Node3 (Replica): `10.10.0.13`
...
각 Replica 노드에서 node1의 키 파일을 SCP로 가져옵니다.

```bash
# 각 Replica 노드(node2, node3)에서 opensql 계정으로 실행
mkdir -p ~/.ssh && chmod 700 ~/.ssh
