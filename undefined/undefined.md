...
| 서버        | 역할       | 설명                                                                       |
| --------- | -------- | ------------------------------------------------------------------------ |
| OwlDB 서버  | 관제 서버    | <p>- OwlDB 애플리케이션이 구동되는 서버<br>- 웹 UI·백엔드 서비스 제공, Agent와 통신해 설치/관제 수행</p> |
| 데이터베이스 서버 | 관제 대상 서버 | <p>- Tibero 데이터베이스와 OwlDB Agent가 설치되는 서버<br>- OwlDB 서버 명령으로 DB 설치/운영</p> |
...
구성 방식별 상세 지원 조건과 사전 준비 사항은 [설치 DB 환경 준비 가이드](undefined.md#W2TxdEHwoC3mStsQfJdo) 또는 [등록 DB 환경 준비 가이드](undefined.md#pvI81bWJlNtie4nv4stI)를 확인해 주세요.
{% endhint %}
...
<table><thead><tr><th width="289">패치명</th><th>패치 내용</th></tr></thead><tbody><tr><td>FS02PS_318447b</td><td>tbboot 시 stdout 닫히지 않는 문제 개선</td></tr><tr><td>FS02PS_339919c</td><td>tbdown 시 switchover immediate 옵션 추가</td></tr></tbody></table>

{% hint style="warning" %}
**주의**

* FS02PS\_318447b 미적용 시 **DB 기동** 기능이 미동작합니다.
* FS02PS\_339919c 미적용 시 **failover**/**switchover** 기능이 미동작합니다.
{% endhint %}
...
| ----------------------------- | --------- | ---------------------------------------------------------- |
| `owldb-cp-installer-*.tar.gz` | OwlDB 서버  | <p>- OwlDB 백엔드·프론트엔드 Docker 이미지<br>- 설치 스크립트</p>           |
| `owldb-dp-installer-*.tar.gz` | 데이터베이스 서버 | <p>- Tibero 설치 스크립트<br>- tbagent 바이너리<br>- 인프라 검증 스크립트</p> |
